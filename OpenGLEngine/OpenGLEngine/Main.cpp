...
void N_Body(ECSWorld& world);
void Buoyancy(ECSWorld& world);
...
int main()
{
	...
	Buoyancy(world); // Question 2: Buoyancy
	N_Body(world); // Question 3: N-Body
	...
	while (!glfwWindowShouldClose(world.data.renderUtil->window->glfwWindow))
	{
		...
		world.data.renderUtil->RenderText("Density: " + std::to_string(world.getSystemManager().getSystem<BuoyancySystem>().liquidDensity), 1400.0f, 500.0f, 1.0f, Color(0.5, 0.5, 0.5, 1));

		world.data.renderUtil->SwapBuffers(world.data.renderUtil->window->glfwWindow);

		// User Control
		if (world.spaceIsPressed % 9 == 1)
		{
			auto newObj = world.createEntity();
			newObj.addComponent<TransformComponent>(world.data.renderUtil->camera.Position +
				Vector3(
					world.data.renderUtil->camera.Front.x * 30,
					10.0f,
					world.data.renderUtil->camera.Front.z * 30
				));
			newObj.addComponent<ParticleComponent>(Vector3(0, 0, 0));
			newObj.addComponent<ForceAccumulatorComponent>();
			newObj.addComponent<GravityForceComponent>();
			newObj.addComponent<BuoyancyComponent>();
			newObj.addComponent<DragForceComponent>(0.5f);

			if (world.spaceIsPressed == 1)
			{
				auto particle2 = world.createEntity();
				particle2.addComponent<TransformComponent>(Vector3(0, 0, 0));
				...
				world.spaceIsPressed += 1;
			}
			world.spaceIsPressed++;
		}

		if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS)
			world.getSystemManager().getSystem<BuoyancySystem>().liquidDensity += 0.1;
		if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS)
			world.getSystemManager().getSystem<BuoyancySystem>().liquidDensity -= 0.1;

		if (world.getSystemManager().getSystem<BuoyancySystem>().liquidDensity <= 0.1)
			world.getSystemManager().getSystem<BuoyancySystem>().liquidDensity = 0.1;
...
void N_Body(ECSWorld& world)
{
	for (int i = 0; i < 10; i++)
	{
		auto particles = world.createEntity();
		particles.addComponent<TransformComponent>(Vector3(RANDOM_FLOAT(0.0f, 50.0f), RANDOM_FLOAT(0.0f, 50.0f), RANDOM_FLOAT(0.0f, 50.0f)));
		particles.addComponent<ParticleComponent>(Vector3(0, 0, 0));
		particles.addComponent<NBodyComponent>();
		particles.addComponent<ForceAccumulatorComponent>();
	}
}
...
void Buoyancy(ECSWorld& world)
{
	...
	particle.addComponent<TransformComponent>(Vector3(0, 10.0f, 0));
	...
	particle.addComponent<BuoyancyComponent>();
	particle.addComponent<DragForceComponent>(0.5f);
}
