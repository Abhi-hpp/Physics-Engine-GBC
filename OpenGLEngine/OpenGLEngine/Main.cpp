...
// We are using the same concept for input in the events loop from SDL engine
bool isSpacePressed = false;

// To be honest I dont like the idea of having a local variable to control whic is the last entity
// but for purposes of finishing in the deadline was the best solution
ECSEntity lastConnectedEntity = ECSEntity();
...
void MakeABunchSprings(ECSWorld& world);
void MakeABunchSpheres(ECSWorld& world);
void MakeABunchBungee(ECSWorld& world); 
void BungeeKeyboardInputSimulation(ECSWorld& world);
void MakeBuoyancy(ECSWorld& world);
void BuoyancyKeyboardInputSimulation(ECSWorld& world);
void makeBuoyancyAtCamera(Reality::ECSWorld& world, Camera& camera, Reality::Vector3& cameraPos);
void MakeBunchaNBody(ECSWorld& world);
...
int main()
{
	...
	//MakeABunchSprings(world);
	//MakeABunchSpheres(world);
	//MakeABunchBungee(world);
	//MakeBuoyancy(world);
	MakeBunchaNBody(world);
	...
	world.getSystemManager().addSystem<BungeePairedSystem>();
	...
	// Hack: init events
	world.getEventManager().emitEvent<ParticleComponent>();
	// game loop
	// -----------
	while (!glfwWindowShouldClose(world.data.renderUtil->window->glfwWindow))
	{
		...
		BungeeKeyboardInputSimulation(world);
		//BuoyancyKeyboardInputSimulation(world);
		...
		world.getSystemManager().getSystem<BungeePairedSystem>().Update(fixedDeltaTime);
...
void Make3Particles(ECSWorld & world)
{
	for (int i = 0; i < 3; i++)
	{
		auto dragforce = world.createEntity();
		dragforce.addComponent<TransformComponent>(Vector3(-100 + 100 * i, 60, -50));
		dragforce.addComponent<ParticleComponent>(Vector3(0, 0, 0));
		dragforce.addComponent<ForceAccumulatorComponent>();
		dragforce.addComponent<GravityForceComponent>();
		dragforce.addComponent<DragForceComponent>(1, 0.5f);
	}
}
...
void MakeABunchSprings(ECSWorld & world)
{
	...
	entityA.addComponent<TransformComponent>(Vector3(-20, 0, -50));
	entityA.addComponent<ParticleComponent>();
	entityA.addComponent<ForceAccumulatorComponent>();
	entityA.addComponent<GravityForceComponent>();

	auto entityB = world.createEntity();
	entityB.addComponent<TransformComponent>(Vector3(-20, 20, -50));
	entityB.addComponent<ParticleComponent>();
	entityB.addComponent<ForceAccumulatorComponent>();
	entityB.addComponent<GravityForceComponent>();

	auto springEntity = world.createEntity();
	springEntity.addComponent<TransformComponent>(Vector3(0, 60, -50));
	springEntity.addComponent<FixedSpringComponent>(10, 20, entityA);

	/*auto springEntity2 = world.createEntity();
	springEntity2.addComponent<TransformComponent>(Vector3(-30, 50, -50));
	springEntity2.addComponent<FixedSpringComponent>(10, 20, entityA);

	auto pairedSpring = world.createEntity();
	pairedSpring.addComponent<PairedSpringComponent>(10, 15, entityA, entityB);*/
}
...
void MakeABunchSpheres(ECSWorld & world)
{
	for (int i = 0; i < 10; i++)
	{
		...
		sphere.addComponent<ParticleComponent>(Vector3(RANDOM_FLOAT(-100, 100), RANDOM_FLOAT(-100, 100), RANDOM_FLOAT(-100, 100)));
		sphere.addComponent<ForceAccumulatorComponent>(RANDOM_FLOAT(1, 10));
		sphere.addComponent<GravityForceComponent>();
		sphere.addComponent<ParticleSphereComponent>(RANDOM_FLOAT(1, 5));
	}
}
...
void MakeABunchBungee(ECSWorld& world)
{
	...
	lastConnectedEntity = entityB;

	auto bungeeEntity = world.createEntity();
	bungeeEntity.addComponent<TransformComponent>(Vector3(0, 60, -50));
	bungeeEntity.addComponent<BungeeComponent>(10, 20, entityA);

	auto bungeeEntity2 = world.createEntity();
	bungeeEntity2.addComponent<TransformComponent>(Vector3(-30, 50, -50));
	bungeeEntity2.addComponent<BungeeComponent>(10, 20, entityA);
...
void BungeeKeyboardInputSimulation(ECSWorld& world)
{
	...
	if ((glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS) && isSpacePressed == false)
	{
		isSpacePressed = true;
		...
		newEntity.addComponent<TransformComponent>(camera.Position + camera.Front);
		newEntity.addComponent<ParticleComponent>(Vector3(0, 0, 0));
		newEntity.addComponent<ForceAccumulatorComponent>();
		newEntity.addComponent<GravityForceComponent>();

		/*auto newbungeeEntity = world.createEntity();
		newbungeeEntity.addComponent<TransformComponent>(Vector3(-40, 60, -40));
		newbungeeEntity.addComponent<BungeeComponent>(10, 20, newEntity);*/

		auto pairedSpring = world.createEntity();
		pairedSpring.addComponent<BungeePairedComponent>(10.0f, 20.0f, lastConnectedEntity, newEntity);

		lastConnectedEntity = newEntity;
	}
	else if ((glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_RELEASE) && isSpacePressed == true)
	{
...
void MakeBuoyancy(ECSWorld& world)
{
	auto buoyn = world.createEntity();
	buoyn.addComponent<TransformComponent>(Vector3(RANDOM_FLOAT(-100, 100), RANDOM_FLOAT(20, 50), RANDOM_FLOAT(-100, 100)));
	buoyn.addComponent<ParticleComponent>(Vector3(0, 0, 0));
	buoyn.addComponent<ForceAccumulatorComponent>();
	buoyn.addComponent<GravityForceComponent>();
	buoyn.addComponent<DragForceComponent>(RANDOM_FLOAT(0.1, 1), 0.0f);
	buoyn.addComponent<BuoyancyComponent>(5.0f, 2.0f);
}
...
void BuoyancyKeyboardInputSimulation(ECSWorld& world)
{
	...
	if ((glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS) && isSpacePressed == false)
	{
		...
		Vector3 cameraPos = camera.Front;

		float distanceToCamera = 50;
		cameraPos.x *= distanceToCamera;
		cameraPos.y *= distanceToCamera;
		cameraPos.z *= distanceToCamera;

		makeBuoyancyAtCamera(world, camera, cameraPos);
	}
	else if ((glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_RELEASE) && isSpacePressed == true)
	{
		isSpacePressed = false;
...
void makeBuoyancyAtCamera(Reality::ECSWorld& world, Camera& camera, Reality::Vector3& cameraPos)
{
	auto newBuoan = world.createEntity();
	newBuoan.addComponent<TransformComponent>(camera.Position + cameraPos);
	newBuoan.addComponent<ParticleComponent>(Vector3(0, 0, 0));
	newBuoan.addComponent<ForceAccumulatorComponent>();
	newBuoan.addComponent<GravityForceComponent>();
	newBuoan.addComponent<DragForceComponent>(RANDOM_FLOAT(0.1, 1), 0.f);
	newBuoan.addComponent<BuoyancyComponent>(5.0f, 2.0f);
}
...
void MakeBunchaNBody(ECSWorld& world)
{
	for (int i = 0; i < 50; ++i)
	{
		...
		nBody.addComponent<TransformComponent>(Vector3(RANDOM_FLOAT(-5, 5), RANDOM_FLOAT(-5, 5), RANDOM_FLOAT(-5, 5)));
		nBody.addComponent<ParticleComponent>(Vector3(RANDOM_FLOAT(-0.5f, 0.5f), RANDOM_FLOAT(-0.5f, 0.5f), RANDOM_FLOAT(-0.5f, 0.5f)));
		nBody.addComponent<ForceAccumulatorComponent>();
		nBody.addComponent<NBodyComponent>();
	}
}
