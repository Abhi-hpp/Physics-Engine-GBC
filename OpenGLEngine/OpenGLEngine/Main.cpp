...
int main()
{
	...
	sphere->addComponent<TransformComponent>(Vector3(RANDOM_FLOAT(-5, 10), RANDOM_FLOAT(10, 20), RANDOM_FLOAT(-3, 3)));
	...
	sphere->addComponent<TriangleComponent>();
	sphere->tag("ball");
	// game loop
	// -----------
	while (!glfwWindowShouldClose(world.data.renderUtil->window->glfwWindow))
	{
		...
		if (glfwGetKey(world.data.renderUtil->window->glfwWindow, GLFW_KEY_E) == GLFW_PRESS)
		{
			sphere->getComponent<TransformComponent>().position = Vector3(RANDOM_FLOAT(-5, 10), RANDOM_FLOAT(10, 20), RANDOM_FLOAT(-3, 3));
			sphere->getComponent<ParticleComponent>().velocity = Vector3(0);
		}
...
void MakeARopeBridge(ECSWorld & world)
{
	...
	ePivot5.addComponent<TransformComponent>(Vector3(3 - 10, 5, 5));
	...
	ePivot6.addComponent<TransformComponent>(Vector3(3 - 10, 5, -5));
	...
	Tri1.addComponent<TriangleComponent>(e5, e6, e2);

	auto Tri2 = world.createEntity();
	Tri2.addComponent<TriangleComponent>(e5, e2, e1);	
	
	auto Tri3 = world.createEntity();
	Tri3.addComponent<TriangleComponent>(e1, e2, e4);

	auto Tri4 = world.createEntity();
	Tri4.addComponent<TriangleComponent>(e1, e4, e3);

	auto Tri5 = world.createEntity();
	Tri5.addComponent<TriangleComponent>(e8, e6, e7);

	auto Tri6 = world.createEntity();
	Tri6.addComponent<TriangleComponent>(e7, e6, e5);

	auto plane1 = world.createEntity();
	plane1.addComponent<PlaneComponent>(Tri1, Tri2);

	auto plane2 = world.createEntity();
	plane2.addComponent<PlaneComponent>(Tri3, Tri4);
