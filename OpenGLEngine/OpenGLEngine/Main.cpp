...
void MakeAFlightSimulator(ECSWorld& world)
{
	...
	flight.addComponent<ModelComponent>("Resources/Models/supermarine-spitfire/spitfire.fbx", Vector3(-90, 180, 0), Vector3(0, -50, 0));
	//flight.addComponent<ModelComponent>("Resources/Models/supermarine-spitfire/spitfire.fbx", Vector3(-90, 0, 0), Vector3(0, -50, 0));
	flight.addComponent<RigidbodyComponent>();
	flight.addComponent<ForceAndTorqueAccumulatorComponent>();
...
void MakeABoatSImulator(ECSWorld& world)
{
	...
	boat.addComponent<TransformComponentV2>(Vector3(0, 20, 0), Vector3(0.1f, 0.1f, 0.1f), Vector3(0, 0, 0));
	...
	boat.addComponent<ForceAndTorqueAccumulatorComponent>(10.f);
	boat.addComponent<DragComponent>(0.5, 0.8);
	...
	engine.addComponent<ThrusterComponent>(boat, Vector3(0, 0, 1), 0, thrustPositiveKeys, thrustNegetiveKeys);

	auto rudder = world.createEntity();
	rudder.addComponent<TransformComponentV2>();
	rudder.addComponent<AeroSurfaceComponent>(boat, Vector3(0, 0, 0), Vector3(0, 0, -100));
	std::vector<int> rudderWingPositiveKeys = { GLFW_KEY_D };
	std::vector<int> rudderWingNegetiveKeys = { GLFW_KEY_A };
	rudder.addComponent<AeroControlComponent>(Vector3(-0.05f, 0, 0), Vector3(0.05f, 0, 0), rudderWingPositiveKeys, rudderWingNegetiveKeys);

	float density = 10.f;
	float offsetValue = 200.0f;
	std::vector<int> leftKey = { GLFW_KEY_A };
	std::vector<int> rightKey = { GLFW_KEY_D };
	Vector3 fBuoyancy = Vector3(6, 6, 6);

	auto frontLeftBuoyancy = world.createEntity();
	frontLeftBuoyancy.addComponent<TransformComponentV2>();
	frontLeftBuoyancy.addComponent<BuoyancyComponent>(boat, Vector3(offsetValue, -5, offsetValue), fBuoyancy, 0.0f, density, rightKey);

	auto frontRightBuoyancy = world.createEntity();
	frontRightBuoyancy.addComponent<TransformComponentV2>();
	frontRightBuoyancy.addComponent<BuoyancyComponent>(boat, Vector3(-offsetValue, -5, offsetValue), fBuoyancy, 0.0f, density, leftKey);

	auto LeftRearBuoyancy = world.createEntity();
	LeftRearBuoyancy.addComponent<TransformComponentV2>();
	LeftRearBuoyancy.addComponent<BuoyancyComponent>(boat, Vector3(offsetValue, 5, -offsetValue), fBuoyancy, 0.0f, density, rightKey);

	auto rightRearBuoyancy = world.createEntity();
	rightRearBuoyancy.addComponent<TransformComponentV2>();
	rightRearBuoyancy.addComponent<BuoyancyComponent>(boat, Vector3(-offsetValue, 5, -offsetValue), fBuoyancy, 0.0f, density, leftKey);
