...
Mix::Entity* CurrentBall = nullptr;
...
int main()
{
	...
	while (!glfwWindowShouldClose(world.data.renderUtil->window->glfwWindow))
	{
		...
		world.data.renderUtil->DrawCube(
			Vector3(0, 0, 0),
			Vector3(100, 100, 100),
			Vector3(0, 0, 0),
			Color::Purple
		...
		// Logic Updates
		if (CanPressSpace && glfwGetKey(world.data.renderUtil->window->glfwWindow, GLFW_KEY_E) == GLFW_PRESS)
		{
			CanPressSpace = false;
			if (CurrentBall)
			{
				world.destroyEntity(*CurrentBall);
			}

			auto Ball = world.createEntity();
			Ball.addComponent<TransformComponent>(Vector3(RANDOM_FLOAT(-5, 10), RANDOM_FLOAT(10, 15), RANDOM_FLOAT(-3, 3)));
			Ball.addComponent<ParticleComponent>();
			Ball.addComponent<ForceAccumulatorComponent>();
			Ball.addComponent<GravityForceComponent>();
			Ball.addComponent<ParticleSphereComponent>();
			Ball.addComponent<PenetrationDeltaMoveComponent>();

			CurrentBall = &Ball;
		}

		if (!CanPressSpace && glfwGetKey(world.data.renderUtil->window->glfwWindow, GLFW_KEY_E) == GLFW_RELEASE)
		{
			CanPressSpace = true;
		}
		...
		world.getSystemManager().getSystem<PlaneSystem>().Update(fixedDeltaTime);
...
void MakeARopeBridge(ECSWorld & world)
{
	...
	plane1.addComponent<ParticleSphereComponent>(0);
	plane1.addComponent<PlaneComponent>(e5, e6, e2, e1);
											
	auto plane2 = world.createEntity();		
	plane2.addComponent<TransformComponent>();
	plane2.addComponent<ParticleSphereComponent>(0);
	plane2.addComponent<PlaneComponent>(e1, e2, e4, e3);
}
