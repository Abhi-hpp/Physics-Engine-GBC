...
int main()
{
	...
	while (!glfwWindowShouldClose(world.data.renderUtil->window->glfwWindow))
	{
		...
		// Rendering Update
		///*** HACK: For the last DrawCall not working on some systems & my water cube
		world.data.renderUtil->DrawCube(Vector3(0, -10, 0), Vector3(300.0f, 1.0f, 500.0f), Vector3(0, 0, 0), Color::Cyan);
		///*** HACK: For the last DrawCall not working on some systems & my water cube
...
void MakeABoatSimulator(ECSWorld& world)
{
	...
	boat.addComponent<TransformComponentV2>(Vector3(0, -5, 0), Vector3(10, 10, 10), Vector3(0, 0, 0));
