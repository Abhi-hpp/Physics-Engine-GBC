...
/*
	Edited by:	Russell Brabers	-	101192571
...
void LoadModels(ECSWorld& world)
{
	world.data.assetLoader->StartModelLoading({
		...
		ModelData("Resources/Models/supermarine-spitfire/spitfire.fbx",
			{{"spitfire_d.png"}}),
		ModelData("Resources/Models/Warship/AmerWarship.obj", {{"GoldReflection.png"}, {"LIGHTWOOD.png"}})
		//ModelData("Resources/Models/Longship/Viking_Longship.fbx", {{"T_Longship_D.jpg", "T_Longship_N.png"}})
		});
}
...
void BoatSimulator(ECSWorld& world)
{
	...
	boat.addComponent<TransformComponentV2>(Vector3(0, 0, 0));
	//boat.addComponent<TransformComponent>(Vector3(0, -100, 0), Vector3(0.1f, 0.1f, 0.1f), Vector3(-90, 0, 0));
	boat.addComponent<ModelComponent>("Resources/Models/Warship/AmerWarship.obj");
	//boat.addComponent<ModelComponent>("Resources/Models/Longship/Viking_Longship.fbx");
	...
	auto engine = world.createEntity();
	engine.addComponent<ThrusterComponent>(boat, Vector3(0, 1, -1), 5);

	auto frontBuoyant = world.createEntity();
	frontBuoyant.addComponent<TransformComponentV2>();
	frontBuoyant.addComponent<BuoyancyComponent>(boat, Vector3(0, 0,10), -4, 4);

	auto backBuoyant = world.createEntity();
	backBuoyant.addComponent<TransformComponentV2>();
	backBuoyant.addComponent<BuoyancyComponent>(boat, Vector3(0, 0, -10), -4, 4);

	auto sail = world.createEntity();
	sail.addComponent<TransformComponentV2>();
	sail.addComponent<AeroSurfaceComponent>(boat, Vector3(0, 0, 0), Vector3(0, 18, 0));
	std::vector<int> sailPositiveKeys = { GLFW_KEY_S };
	std::vector<int> sailNegetiveKeys = { GLFW_KEY_W };
	...
	auto rudder = world.createEntity();
	rudder.addComponent<TransformComponentV2>();
	rudder.addComponent<AeroSurfaceComponent>(boat, Vector3(0, 0, 0), Vector3(0, 0, -15));
	std::vector<int> rudderWingPositiveKeys = { GLFW_KEY_D };
	std::vector<int> rudderWingNegetiveKeys = { GLFW_KEY_A };
	rudder.addComponent<AeroControlComponent>(
		Vector3(-0.01f, 0, 0),
