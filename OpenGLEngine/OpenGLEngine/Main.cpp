...
int main()
{
	...
	world.data.renderUtil->camera.Position = Vector3(0, 0.0f, 20.0f);
	world.data.renderUtil->SetFOV(100);
	// Create entities
...
void LoadModels(ECSWorld& world)
{
	world.data.assetLoader->StartModelLoading({
		...
		ModelData("Resources/Models/supermarine-spitfire/spitfire.fbx",
			{{"spitfire_d.png"}}),
		ModelData("Resources/Models/GalleonOGA.obj")
		});
}
...
void MakeABoatSimulator(ECSWorld& world)
{
	...
	boat.addComponent<TransformComponentV2>(Vector3(0, 0, 10), Vector3(5.f, 5.f, 5.f), Vector3(0, 0, 0));
	boat.addComponent<ModelComponent>("Resources/Models/GalleonOGA.obj", Vector3(0, 180, 0), Vector3(0, -5.0f, 0));
	...
	boat.addComponent<DragComponent>(0.3, 0.9);
	...
	buoyancyFront.addComponent<BuoyancyComponent>(boat, 4.f, 2.f, Vector3(-5, 0, 0));

	auto buoyancyRear = world.createEntity();
	buoyancyRear.addComponent<TransformComponentV2>();
	buoyancyRear.addComponent<BuoyancyComponent>(boat, 8.f, 2.f, Vector3(5, 0, 0));

	auto leftRotating = world.createEntity();
	leftRotating.addComponent<TransformComponentV2>();
	leftRotating.addComponent<AeroSurfaceComponent>(boat, Vector3(5, 0, 0), Vector3(-5, 0, 0));
	...
	rightRotating.addComponent<AeroSurfaceComponent>(boat, Vector3(5, 0, 0), Vector3(5, 0, 0));
...
void SetupLights(ECSWorld& world)
{
	...
	l.addComponent<DynamicDirectionalLightComponent>(Color(1, 1, 1), Color(0.3, 0.3, 0.3), Color(0.0, 0.1, 0.1));
