...
int main()
{
	...
	world.data.renderUtil->camera.Position = Vector3(0, 15.0f, 300.0f);
	...
	while (!glfwWindowShouldClose(world.data.renderUtil->window->glfwWindow))
	{
		...
		if (!Pressed && glfwGetKey(world.data.renderUtil->window->glfwWindow, GLFW_KEY_SPACE) == GLFW_PRESS)
		{
			Pressed = true;
			auto sphere = world.createEntity();
			sphere.addComponent<TransformComponent>(world.data.renderUtil->camera.Position);
			...
			sphere.addComponent<DragForceComponent>(0.4, 0);

			auto pairedSpring = world.createEntity();
			Vector3 relativePosition = world.data.renderUtil->camera.Position - lastSphere.getComponent<TransformComponent>().position;
			float length = glm::length(relativePosition);
			pairedSpring.addComponent<PairedSpringComponent>(2.0f, length, sphere, lastSphere);

			lastSphere = sphere;

			auto sphere2 = world.createEntity();
			sphere2.addComponent<TransformComponent>(world.data.renderUtil->camera.Position + world.data.renderUtil->camera.Front * 100.0f);

			sphere2.addComponent<ParticleComponent>(Vector3(0, 0, 0));
			sphere2.addComponent<ForceAccumulatorComponent>(1.0f);
			sphere2.addComponent<ParticleSphereComponent>(3);
			sphere2.addComponent<GravityForceComponent>(0);
			sphere2.addComponent<DragForceComponent>(1, 0);
			sphere2.addComponent<BuoyancyComponent>(10, 0.001, 0, 1000.0f);
		}

		else if (Pressed && glfwGetKey(world.data.renderUtil->window->glfwWindow, GLFW_KEY_SPACE) == GLFW_RELEASE)
		{
			Pressed = false;
		}
...
{
	auto sphere = world.createEntity();
	sphere.addComponent<TransformComponent>(glm::vec3(40, 50, 0));
	sphere.addComponent<ParticleComponent>();

	auto sphere2 = world.createEntity();
	sphere2.addComponent<TransformComponent>(Vector3(40, 0, RANDOM_FLOAT(-5, 5)));
	sphere2.addComponent<ParticleComponent>(Vector3(RANDOM_FLOAT(-5, 5), 0, RANDOM_FLOAT(-5, 5)));
	sphere2.addComponent<ForceAccumulatorComponent>(RANDOM_FLOAT(1, 4));
	sphere2.addComponent<GravityForceComponent>();
	sphere2.addComponent<ParticleSphereComponent>(RANDOM_FLOAT(1, 3));
	sphere2.addComponent<DragForceComponent>(1, 0);
	...
	auto pairedSpring = world.createEntity();
	pairedSpring.addComponent<PairedSpringComponent>(2.0f, 0.0f, sphere, sphere2);
}

void MakeBuoyancy(ECSWorld& world)
{
	auto sphere = world.createEntity();
	sphere.addComponent<TransformComponent>(Vector3(RANDOM_FLOAT(-25, -20), 0, RANDOM_FLOAT(-5, 5)));
	sphere.addComponent<ParticleComponent>(Vector3(0, 0, 0));
	...
	sphere.addComponent<DragForceComponent>(1, 0);
	sphere.addComponent<BuoyancyComponent>(10, 0.001, 0, 1000.0f);
...
void MakeNBody(ECSWorld & world)
{
	...
	{
		auto sphere = world.createEntity();
		sphere.addComponent<TransformComponent>(Vector3(RANDOM_FLOAT(-20, 20), 200, RANDOM_FLOAT(-20, 20)));
		sphere.addComponent<ParticleComponent>(Vector3(RANDOM_FLOAT(-100, 100), RANDOM_FLOAT(-50, 50), RANDOM_FLOAT(-100, 100)));
		sphere.addComponent<ForceAccumulatorComponent>(1.0f);
		sphere.addComponent<ParticleSphereComponent>(RANDOM_FLOAT(1, 10));
		sphere.addComponent<GravityForceComponent>(0);
		sphere.addComponent<NBodyComponent>();
	}
}
