...
void MakeDragParticles(ECSWorld& world);
void MakeSpringParticles(ECSWorld& world);
ECSEntity MakeBungeeParticles(ECSWorld& world);
ECSEntity MakeAdditionalBungee(ECSWorld& world, ECSEntity entityA);
bool MakeBuoyancyParticles(ECSWorld& world);
void MakeAdditionalBuoyancy(ECSWorld& world, float buoyancyDensity);
...
int main()
{
	...
	ECSEntity currentBungeeEntity;
	bool buoyancySimulation = false;

	SetupLights(world);
	//MakeABunchaObjects(world);
	//MakeFireworks(world);
	//MakeDragParticles(world);
	//MakeSpringParticles(world);


	// Assignment 1 Functions

	//currentBungeeEntity = MakeBungeeParticles(world);
	//buoyancySimulation = MakeBuoyancyParticles(world);
	MakeNBody(world);
	...
	while (!glfwWindowShouldClose(world.data.renderUtil->window->glfwWindow))
	{
		...
		if (glfwGetKey(world.data.renderUtil->window->glfwWindow, GLFW_KEY_SPACE) == GLFW_PRESS && spawning)
		{
			if (buoyancySimulation)
			{
				MakeAdditionalBuoyancy(world, world.getSystemManager().getSystem<BuoyancySystem>().currentBuoyancyValue);
			}
			else
			{
				currentBungeeEntity = MakeAdditionalBungee(world, currentBungeeEntity);
			}
			spawning = false;
		}
		...
		world.getSystemManager().getSystem<ParticleContactResolutionSystem>().Update(deltaTime);
		...
		if (DEBUG_LOG_LEVEL > 0)
		{
			...
			if(buoyancySimulation)
				world.data.renderUtil->RenderText("Current Liquid Density: " + std::to_string(world.getSystemManager().getSystem<BuoyancySystem>().currentBuoyancyValue), 800.0f, 1000.0f, 0.5, Color::Blue);
		}
...
void MakeDragParticles(ECSWorld & world)
{
	...
	{
		auto fireworks = world.createEntity();
		fireworks.addComponent<TransformComponent>(Vector3(-100 + 100 * i, 40, -50));
		fireworks.addComponent<ParticleComponent>(Vector3(0, 0, 0));
		fireworks.addComponent<ForceAccumulatorComponent>();
		fireworks.addComponent<GravityForceComponent>();
		fireworks.addComponent<DragForceComponent>(i == 0 ? 0 : 1, i == 2 ? 1 : 0);
	}
}
...
void MakeSpringParticles(ECSWorld & world)
{
	...
	entityB.addComponent<TransformComponent>(Vector3(0, 0, -50));

	auto bungee = world.createEntity();
	bungee.addComponent<BungeeComponent>(10, 50, entityA, entityB);

	//auto spring1 = world.createEntity();
	//spring1.addComponent<TransformComponent>(Vector3(0, 60, -50));
	//spring1.addComponent<FixedSpringComponent>(10, 10, entityA);
	
	//auto spring2 = world.createEntity();
	//spring2.addComponent<TransformComponent>(Vector3(-30, 50, -50));
	//spring2.addComponent<FixedSpringComponent>(10, 10, entityA);
	
	//auto pairedSpring = world.createEntity();
	//pairedSpring.addComponent<PairedSpringComponent>(10, 10, entityA, entityB);
}
...
ECSEntity MakeBungeeParticles(ECSWorld & world)
{
	...
	auto bungee = world.createEntity();
	bungee.addComponent<BungeeComponent>(1, 100, entityA, entityB);
	return entityA;
...
{
	auto entityB = world.createEntity();
	entityB.addComponent<TransformComponent>(world.data.renderUtil->camera.Position);
	...
	auto bungee = world.createEntity();
	bungee.addComponent<BungeeComponent>(1, 100, entityB, entityA);
...
bool MakeBuoyancyParticles(ECSWorld & world)
{
	...
	entity.addComponent<TransformComponent>(Vector3(0, 500, -50));
	entity.addComponent<ParticleComponent>();
	entity.addComponent<ForceAccumulatorComponent>();
	entity.addComponent<GravityForceComponent>();
	entity.addComponent<BuoyancyComponent>(100, 10, 0, 100);

	auto entity1 = world.createEntity();
	entity1.addComponent<TransformComponent>(Vector3(20, 300, -50));
	entity1.addComponent<ParticleComponent>();
	entity1.addComponent<ForceAccumulatorComponent>();
	entity1.addComponent<GravityForceComponent>();
	entity1.addComponent<BuoyancyComponent>(100, 10, 0, 100);
	return true;
}
...
{
	auto entity = world.createEntity();
	entity.addComponent<TransformComponent>(world.data.renderUtil->camera.Position);
	entity.addComponent<ParticleComponent>();
	entity.addComponent<ForceAccumulatorComponent>();
	entity.addComponent<GravityForceComponent>();
	entity.addComponent<BuoyancyComponent>(100, 10, 0, buoyancyDensity);
}

void MakeNBody(ECSWorld & world)
{
	//auto mainEntity = world.createEntity();
	//mainEntity.addComponent<TransformComponent>(Vector3(0, 0, -50));
	//mainEntity.addComponent<ParticleComponent>();
	//mainEntity.addComponent<ForceAccumulatorComponent>();
	//mainEntity.addComponent<NBodyComponent>(100);

	for (int i = 0; i < 10; i++)
	{
		auto entity = world.createEntity();
		entity.addComponent<TransformComponent>(Vector3(RANDOM_FLOAT(-50, 50), RANDOM_FLOAT(-50, 50), -50));
		entity.addComponent<ParticleComponent>(Vector3(RANDOM_FLOAT(-5, 5), RANDOM_FLOAT(-5, 5), 0));
		entity.addComponent<ForceAccumulatorComponent>();
		entity.addComponent<NBodyComponent>(RANDOM_FLOAT(1, 10));
	}
...
void MakeABunchaSpheres(ECSWorld& world)
{
	for (int i = 0; i < 10; i++)
	{
		...
		sphere.addComponent<ParticleComponent>(Vector3(RANDOM_FLOAT(-10, 10), RANDOM_FLOAT(-10, 10), RANDOM_FLOAT(-10, 10)));
