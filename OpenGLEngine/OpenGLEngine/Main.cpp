...
int main()
{
	...
	world.getSystemManager().addSystem<TriCollideSystem>();
	...
	while (!glfwWindowShouldClose(world.data.renderUtil->window->glfwWindow))
	{
		...
		world.getSystemManager().getSystem<TriCollideSystem>().Update(deltaTime);
		// Update Transform
...
void MakeARopeBridge(ECSWorld& world)
{
	...
	cable1.addComponent<CableComponent>(ePivot1, e1, 20, 1);  // LONG CableS
	...
	rod7.addComponent<RodComponent>(e5, e1, 10);
	auto rod8 = world.createEntity();
	rod8.addComponent<RodComponent>(e6, e2, 10);

	// diagonal rods
	auto rod9 = world.createEntity();
	rod9.addComponent<RodComponent>(e1, e4, 10 * pow(2.0f, 0.5f));
	auto rod10 = world.createEntity();
	rod10.addComponent<RodComponent>(e2, e3, 10 * pow(2.0f, 0.5f));
	auto rod11 = world.createEntity();
	rod11.addComponent<RodComponent>(e6, e1, 10 * pow(2.0f, 0.5f));
	auto rod12 = world.createEntity();
	rod12.addComponent<RodComponent>(e5, e2, 10 * pow(2.0f, 0.5f));

	// OUR RODS (phrasing BOOM!)

	auto rod13 = world.createEntity();
	rod13.addComponent<RodComponent>(e5, e7, 10);
	...
	rod16.addComponent<RodComponent>(e5, e8, 10 * pow(2.0f, 0.5f));
	auto rod17 = world.createEntity();
	rod17.addComponent<RodComponent>(e5, e8, 10 * pow(2.0f, 0.5f));


	// TriangleColliders

	auto tri1 = world.createEntity();
	tri1.addComponent<TriCollideComponent>(e5, e7, e8);

	tri1.addComponent<ParticleComponent>(Vector3(0, 0, 0));
	//tri1.addComponent<TransformComponent>(Vector3(0, 0, 0));

	auto tri2 = world.createEntity();
	tri2.addComponent<TriCollideComponent>(e6, e5, e8);

	tri2.addComponent<ParticleComponent>(Vector3(0, 0, 0));


	///////////////////////////////////////////////////////////
	auto tri3 = world.createEntity();
	tri3.addComponent<TriCollideComponent>(e4, e3, e2);

	tri3.addComponent<ParticleComponent>(Vector3(0, 0, 0));

	auto tri4 = world.createEntity();
	tri4.addComponent<TriCollideComponent>(e2, e3, e1);

	tri4.addComponent<ParticleComponent>(Vector3(0, 0, 0));
	///////////////////////////////////////////////////////////

	auto tri5 = world.createEntity();
	tri5.addComponent<TriCollideComponent>(e5, e6, e2);

	tri5.addComponent<ParticleComponent>(Vector3(0, 0, 0));

	auto tri6 = world.createEntity();
	tri6.addComponent<TriCollideComponent>(e1, e5, e2);

	tri6.addComponent<ParticleComponent>(Vector3(0, 0, 0));


	//auto testBall = world.createEntity();
	//testBall.addComponent<ParticleSphereComponent>(1.0f);
	//testBall.addComponent<TransformComponent>(Vector3(-9, 8, -1));
	//testBall.addComponent<GravityForceComponent>(1.0f);
	//testBall.addComponent<ForceAccumulatorComponent>();
	//testBall.addComponent<ParticleComponent>();

	//input(world, testBall);


}

int ballNumber = 0;
string tag = to_string(ballNumber);
...
void input(ECSWorld& world)
{
	...
	if (isBall == false)
	{
		...
		testBall1.addComponent<TransformComponent>(Vector3(RANDOM_FLOAT(-10, 10), 10, -1));
	...
	if (glfwGetKey(window, GLFW_KEY_E) == GLFW_RELEASE && pressed)
	{
		...
		ballNumber++;
		tag = to_string(ballNumber);


		auto testBall1 = world.createEntity();
		testBall1.tag(tag);
		testBall1.addComponent<ParticleSphereComponent>(1.0f);
		testBall1.addComponent<TransformComponent>(Vector3(RANDOM_FLOAT(-10, 10), RANDOM_FLOAT(0, 10), -1));
		testBall1.addComponent<GravityForceComponent>(1.0f);
		testBall1.addComponent<ForceAccumulatorComponent>();
		testBall1.addComponent<ParticleComponent>();
