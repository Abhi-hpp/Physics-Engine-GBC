...
namespace Reality
{
	...
	void ParticleSphereSystem::Update(float deltaTime)
	{
		for (auto e : getEntities())
		{
			...
			{
				getWorld().data.renderUtil->DrawSphere(e.getComponent<TransformComponent>().position, e.getComponent<ParticleSphereComponent>().radius, Color::Orange);
			}
		}
		if (getEntities().size() > 1)
		{
			for (int i = 0; i < getEntities().size() - 1; i++)
			{
				for (int j = i + 1; j < getEntities().size(); j++)
				{
					...
					{
						getWorld().data.renderUtil->DrawLine(getEntities()[i].getComponent<TransformComponent>().position, getEntities()[j].getComponent<TransformComponent>().position, Color::Red);
					}
				}
			}
		}
	...
	bool ParticleSphereSystem::CheckCollision(ECSEntity& sphereA, ECSEntity& sphereB)
	{
		...
		{
			getWorld().getEventManager().emitEvent<ParticleContactEvent>(sphereA, sphereB, 1.0f, glm::normalize(posA - posB), r1 + r2 - glm::distance(posA, posB));
			return true;
		}
