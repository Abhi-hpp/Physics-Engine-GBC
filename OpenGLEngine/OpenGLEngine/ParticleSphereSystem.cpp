...
namespace Reality
{
	...
	void ParticleSphereSystem::Update(float deltaTime)
	{
		...
		getWorld().data.renderUtil->DrawCube(Vector3(0, 0, 0), Vector3(20, 20, 20), Vector3(0, 0, 0), Color::Magenta);
		...
		for (auto e : getEntities())
		{
			...
			if (abs(transform.position.y) + sphere.radius >= 10)
			{
				...
				getWorld().data.renderUtil->DrawLine(transform.position - normal * sphere.radius,
					transform.position - normal * (sphere.radius - penetration), Color::Red);
			}
