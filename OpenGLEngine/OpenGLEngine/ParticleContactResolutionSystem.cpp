...
namespace Reality
{
	...
	float ParticleContactResolutionSystem::CalculateSeparationVelocity(ParticleContactEvent & contact)
	{
		...
		Vector3 velocityB;
		if (contact.entityB.hasComponent<TriangleComponent>())
		{
			auto triangle = contact.entityB.getComponent<TriangleComponent>();
			velocityB = triangle.vertex_1.hasComponent<ParticleComponent>() ?
				triangle.vertex_1.getComponent<ParticleComponent>().velocity : Vector3(0, 0, 0);
		}
		else
		{
	...
	float ParticleContactResolutionSystem::CalculateActualPenetration(ParticleContactEvent & contact)
	{
		...
		{
			auto triangle = contact.entityB.getComponent<TriangleComponent>();
			if (triangle.vertex_1.hasComponent<PenetrationDeltaMoveComponent>())
			{
				Vector3 deltaMove = triangle.vertex_1.getComponent<PenetrationDeltaMoveComponent>().deltaMove;
				actualPenetration += glm::dot(deltaMove, contact.normal);
			}
		}
		else
	...
	void ParticleContactResolutionSystem::ResolveVelocity(ParticleContactEvent & contact, float deltaTime)
	{
		...
		{
			auto triangle = contact.entityB.getComponent<TriangleComponent>();
			if (triangle.vertex_1.hasComponent<ParticleComponent>())
			{
				relativeAccelaration -= triangle.vertex_1.getComponent<ParticleComponent>().acceleration;
			}
		}
		else
		{
		...
		{
			auto triangle = contact.entityB.getComponent<TriangleComponent>();
			invMB = triangle.vertex_1.hasComponent<ForceAccumulatorComponent>() ?
				triangle.vertex_1.getComponent<ForceAccumulatorComponent>().inverseMass : 0;
		}
		else
		{
		...
		if (contact.entityB.hasComponent<TriangleComponent>())
		{
			...
			if (triangle.vertex_1.hasComponent<ParticleComponent>())
				triangle.vertex_1.getComponent<ParticleComponent>().velocity -= impulsePerIMass * invMB;
			if (triangle.vertex_2.hasComponent<ParticleComponent>())
				triangle.vertex_2.getComponent<ParticleComponent>().velocity -= impulsePerIMass * invMB;
			if (triangle.vertex_3.hasComponent<ParticleComponent>())
				triangle.vertex_3.getComponent<ParticleComponent>().velocity -= impulsePerIMass * invMB;
		}
		else
		{
			if (contact.entityB.hasComponent<ParticleComponent>())
	...
	void ParticleContactResolutionSystem::ResolveInterPenetration(ParticleContactEvent & contact)
	{
		...
		{
			auto triangle = contact.entityB.getComponent<TriangleComponent>();
			invMassB = triangle.vertex_1.hasComponent<ForceAccumulatorComponent>() ?
				triangle.vertex_1.getComponent<ForceAccumulatorComponent>().inverseMass : 0;
		}
		else
		{
		...
		if (contact.entityB.hasComponent<TriangleComponent>())
		{
			auto triangle = contact.entityB.getComponent<TriangleComponent>();
			Vector3 deltaMove = movePerUnitIMass * invMassB;
			triangle.vertex_1.getComponent<TransformComponent>().position -= movePerUnitIMass * invMassB;
			if (triangle.vertex_1.hasComponent<PenetrationDeltaMoveComponent>())
			{
				triangle.vertex_1.getComponent<PenetrationDeltaMoveComponent>().deltaMove -= deltaMove;
			}

			triangle.vertex_2.getComponent<TransformComponent>().position -= movePerUnitIMass * invMassB;
			if (triangle.vertex_2.hasComponent<PenetrationDeltaMoveComponent>())
			{
				triangle.vertex_2.getComponent<PenetrationDeltaMoveComponent>().deltaMove -= deltaMove;
			}

			triangle.vertex_3.getComponent<TransformComponent>().position -= movePerUnitIMass * invMassB;
			if (triangle.vertex_3.hasComponent<PenetrationDeltaMoveComponent>())
			{
				triangle.vertex_3.getComponent<PenetrationDeltaMoveComponent>().deltaMove -= deltaMove;
			}
