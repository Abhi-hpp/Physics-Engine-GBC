...
namespace Reality
{
	...
	void ParticleContactResolutionSystem::ResolveVelocity(ParticleContactEvent & contact, float deltaTime)
	{
		...
		float newSeparationVelocity = -contact.restitution * separatingVelocity;
		float deltaVel = newSeparationVelocity - separatingVelocity;
		...
		Vector3 impulsePerMass = contact.normal * impulse;

		if (contact.entityA.hasComponent<ParticleComponent>())
		{
			contact.entityA.getComponent<ParticleComponent>().velocity += impulsePerMass * invM1;
		}

		if (contact.entityB.hasComponent<ParticleComponent>())
		{
			contact.entityB.getComponent<ParticleComponent>().velocity -= impulsePerMass * invM2;
		}
