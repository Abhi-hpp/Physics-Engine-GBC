// PHYSICS ENGINE
// ASSIGNMENT 1 by Amer Ali Mohammed (ID: 101206769) and Talhah Ramadan Salick (ID: 101214166)
...
// Bungee Chord 
void MakeABungeeChord(ECSWorld& world);

// Buoyancy 
void MakeBuoyancyObjects(ECSWorld& world);

// NBody
void MakeNBody(ECSWorld& world);

// bungee global variables
vector<ECSEntity> vbungeeChords;
bool spawnBungee = false;

// buoyancy global variables
vector<ECSEntity> vBuoyancyObj;
bool spawnBuoyancyObj = false;
...
int main()
{
	...
	world.data.renderUtil->camera.Position = Vector3(0, 20.0f, 30.0f);
	...
	//---------------------------------------------------------------------------------
	//Uncomment to run Bungee chord Simulation
	//MakeABungeeChord(world);

	//---------------------------------------------------------------------------------
	//Uncomment to run Buoyancy Simulation
	MakeBuoyancyObjects(world);

	//---------------------------------------------------------------------------------
	//Uncomment to run NBody Simulation
	//MakeNBody(world);
	...
	while (!glfwWindowShouldClose(world.data.renderUtil->window->glfwWindow))
	{
		...
		// event handling for Bungee
		if (glfwGetKey(world.data.renderUtil->window->glfwWindow, GLFW_KEY_SPACE) == GLFW_PRESS && spawnBungee == false)
		{
			// create new chords if the Bungee vector is empty.
			if (!vbungeeChords.empty())
			{
				auto particle = world.createEntity();
				particle.addComponent<TransformComponent>(world.data.renderUtil->camera.Position + Vector3(0,0,-10));
				...
				auto previousParticle = vbungeeChords[vbungeeChords.size()-1];

				vbungeeChords.push_back(particle);

				auto pairedBungee = world.createEntity();
				pairedBungee.addComponent<PairedBungeeComponent>(20.0f, 20.0f, previousParticle, particle);
				spawnBungee = true;
			}
		}
		else if (glfwGetKey(world.data.renderUtil->window->glfwWindow, GLFW_KEY_SPACE) == GLFW_RELEASE && spawnBungee == true)
		{
			spawnBungee = false;
		}

		//event handling for buoyancy
		if (glfwGetKey(world.data.renderUtil->window->glfwWindow, GLFW_KEY_SPACE) == GLFW_PRESS && spawnBuoyancyObj == false)
		{
			// create new objects if the vector is empty.
			if (!vBuoyancyObj.empty())
			{
				auto particle1 = world.createEntity();
				particle1.addComponent<TransformComponent>(world.data.renderUtil->camera.Position + Vector3(0, 5, -20));
				...
				auto& buoyancy = vBuoyancyObj[0].getComponent<BuoyancyComponent>();
				particle1.addComponent<BuoyancyComponent>(buoyancy.dimensions, buoyancy.liquidHeight, buoyancy.liquidDensity);

				spawnBuoyancyObj = true;
			}
		}
		else if (glfwGetKey(world.data.renderUtil->window->glfwWindow, GLFW_KEY_SPACE) == GLFW_RELEASE && spawnBuoyancyObj == true)
		{
			spawnBuoyancyObj = false;
		}
		// Event handling to increase or decrease liquid floor buoyancy
		if (glfwGetKey(world.data.renderUtil->window->glfwWindow, GLFW_KEY_UP) == GLFW_PRESS )
		{
			if (!vBuoyancyObj.empty())
			{
				for (auto floaty  : vBuoyancyObj)
				{
...
void MakeABungeeChord(ECSWorld & world)
{
	...
	particle1.addComponent<ForceAccumulatorComponent>();
	particle1.addComponent<GravityForceComponent>();
	vbungeeChords.push_back(particle1);
	...
	particle2.addComponent<ForceAccumulatorComponent>();
	particle2.addComponent<GravityForceComponent>();
	vbungeeChords.push_back(particle2);
...
void MakeBuoyancyObjects(ECSWorld & world)
{
		...
		auto particle1 = world.createEntity();
		particle1.addComponent<TransformComponent>(Vector3(RANDOM_FLOAT(-40, 40), 20, RANDOM_FLOAT(-40, 40)));
		particle1.addComponent<ParticleComponent>(Vector3(0, 0, 0));
		particle1.addComponent<ForceAccumulatorComponent>();
		particle1.addComponent<GravityForceComponent>();
		particle1.addComponent<DragForceComponent>(0.50, 0.0f);
		particle1.addComponent<BuoyancyComponent>();
		vBuoyancyObj.push_back(particle1);
...
void MakeNBody(ECSWorld & world)
{
	...
	{
		auto particle1 = world.createEntity();
		particle1.addComponent<TransformComponent>(Vector3(RANDOM_FLOAT(-40, 40), RANDOM_FLOAT(-20, 20), RANDOM_FLOAT(-40, 40)));
		particle1.addComponent<ParticleComponent>(Vector3(RANDOM_FLOAT(-1, 1), RANDOM_FLOAT(-1, 1), RANDOM_FLOAT(-1, 1)));
		particle1.addComponent<ForceAccumulatorComponent>(RANDOM_FLOAT(3, 8));
