...
void LoadModels(ECSWorld& world)
{
	world.data.assetLoader->StartModelLoading({
		...
		ModelData("Resources/Models/supermarine-spitfire/spitfire.fbx",
			{{"spitfire_d.png"}}),
		ModelData("Resources/Models/Ship_free_FBX/Ship_free.fbx")
		});
...
void MakeABoatSimulator(ECSWorld& world)
{
	...
	water.addComponent<TransformComponent>(Vector3(0, 0, 0), Vector3(1, 1, 1), Vector3(0, 0, 0));
	water.addComponent<ForceAccumulatorComponent>();
	water.addComponent<ParticleComponent>(Vector3(0, 30, 0));
	water.addComponent<GravityForceComponent>();
	water.addComponent<BuoyancyComponent>();*/

	
	auto boat = world.createEntity();
	boat.addComponent<TransformComponentV2>(Vector3(0, 2520, -50), Vector3(0.1f, 0.1f, 0.1f), Vector3(0, 180, 0));
	boat.addComponent<ModelComponent>("Resources/Models/Ship_free_FBX/Ship_free.fbx", Vector3(-90, 0, 0), Vector3(0, 50, 0));
	boat.addComponent<RigidbodyComponent>();
	boat.addComponent<GravityForceComponent>();
	boat.addComponent<ForceAndTorqueAccumulatorComponent>(10);
	boat.addComponent<DragComponent>(0.3, 0.5);
	boat.addComponent<FollowCameraComponent>();
	float volumeSide = 4.0f;
	float densitySide = 0.5f;
	auto engine = world.createEntity();
	std::vector<int>enginePositive = { GLFW_KEY_W };
	std::vector<int>engineNegative = { GLFW_KEY_S };
	engine.addComponent<ThrusterComponent>(boat, Vector3(0, 0, 1), 50,enginePositive,engineNegative);
	
	auto frontleftNode = world.createEntity();
	frontleftNode.addComponent<TransformComponentV2>();
	frontleftNode.addComponent<BuoyancyComponent>(boat, densitySide,Vector3(50,20,100),1.0f,Vector3(volumeSide, volumeSide, volumeSide));

	auto frontrightNode = world.createEntity();
	frontrightNode.addComponent<TransformComponentV2>();
	frontrightNode.addComponent<BuoyancyComponent>(boat, densitySide, Vector3(-50, 20, 100), 1.0f, Vector3(volumeSide, volumeSide, volumeSide));

	auto backleftNode = world.createEntity();
	backleftNode.addComponent<TransformComponentV2>();
	backleftNode.addComponent<BuoyancyComponent>(boat, densitySide-0.05f, Vector3(50, 20, -100), 1.0f, Vector3(volumeSide, volumeSide, volumeSide));

	auto backrightNode = world.createEntity();
	backrightNode.addComponent<TransformComponentV2>();
	backrightNode.addComponent<BuoyancyComponent>(boat, densitySide-0.05f, Vector3(-50, 20, -100), 1.0f, Vector3(volumeSide, volumeSide, volumeSide));
