...
namespace Reality
{
	...
	float ParticleContactResolutionSystem::CalculateSeparationVelocity(ParticleContactEvent& contact)
	{
		...
		if (contact.entityB.hasComponent<PlaneComponent>())
		{
			auto plane = contact.entityB.getComponent<PlaneComponent>();
			velocityB = plane.point1.hasComponent<PlaneComponent>() ?
				plane.point1.getComponent<ParticleComponent>().velocity : Vector3(0, 0, 0);
		}
		else
		{
	...
	float ParticleContactResolutionSystem::CalculateActualPenetration(ParticleContactEvent& contact)
	{
		...
		if (contact.entityB.hasComponent<PlaneComponent>())
		{
			...
			if (plane.point1.hasComponent<PenetrationDeltaMoveComponent>())
			{
				Vector3 deltaMove = plane.point1.getComponent<PenetrationDeltaMoveComponent>().deltaMove;
				actualPenetration += glm::dot(deltaMove, contact.normal);
			}
	...
	void ParticleContactResolutionSystem::ResolveVelocity(ParticleContactEvent& contact, float deltaTime)
	{
		...
		{
			auto plane = contact.entityB.getComponent<PlaneComponent>();
			relativeAccelaration -= plane.point1.hasComponent<PlaneComponent>() ?
				plane.point1.getComponent<ParticleComponent>().acceleration : Vector3(0, 0, 0);
		}
		else {
		...
		{
			auto plane = contact.entityB.getComponent<PlaneComponent>();
			invMB = plane.point1.hasComponent<ForceAccumulatorComponent>() ?
				plane.point1.getComponent<ForceAccumulatorComponent>().inverseMass : 0;
		...
		if (contact.entityB.hasComponent<PlaneComponent>())
		{
			if (contact.entityB.getComponent<PlaneComponent>().point1.hasComponent<ParticleComponent>())
				contact.entityB.getComponent<PlaneComponent>().point1.getComponent<ParticleComponent>().velocity -= impulsePerIMass * invMB;

			if (contact.entityB.getComponent<PlaneComponent>().point2.hasComponent<ParticleComponent>())
			{
				contact.entityB.getComponent<PlaneComponent>().point2.getComponent<ParticleComponent>().velocity -= impulsePerIMass * invMB;
			}

			if (contact.entityB.getComponent<PlaneComponent>().point3.hasComponent<ParticleComponent>())
			{
				contact.entityB.getComponent<PlaneComponent>().point3.getComponent<ParticleComponent>().velocity -= impulsePerIMass * invMB;
			}
		}
		else {
			if (contact.entityB.hasComponent<ParticleComponent>())
	...
	void ParticleContactResolutionSystem::ResolveInterPenetration(ParticleContactEvent& contact)
	{
		...
		if (contact.entityB.hasComponent<PlaneComponent>())
		{
			invMassB = contact.entityB.getComponent<PlaneComponent>().point1.hasComponent<ForceAccumulatorComponent>() ?
				contact.entityB.getComponent<PlaneComponent>().point1.getComponent<ForceAccumulatorComponent>().inverseMass : 0;
		...
		if (contact.entityB.hasComponent<PlaneComponent>())
		{
			...
			if (plane.point1.hasComponent<TransformComponent>())
			{
				plane.point1.getComponent<TransformComponent>().position -= movePerUnitIMass * invMassB;
				if (plane.point1.hasComponent<PenetrationDeltaMoveComponent>())
				{
					plane.point1.getComponent<PenetrationDeltaMoveComponent>().deltaMove -= deltaMove;
				}
			}
			if (plane.point2.hasComponent<TransformComponent>())
			{
				plane.point2.getComponent<TransformComponent>().position -= movePerUnitIMass * invMassB;
				if (plane.point2.hasComponent<PenetrationDeltaMoveComponent>())
				{
					plane.point2.getComponent<PenetrationDeltaMoveComponent>().deltaMove -= deltaMove;
				}
			}
			if (plane.point3.hasComponent<TransformComponent>())
			{
				plane.point3.getComponent<TransformComponent>().position -= movePerUnitIMass * invMassB;
				if (plane.point3.hasComponent<PenetrationDeltaMoveComponent>())
				{
					plane.point3.getComponent<PenetrationDeltaMoveComponent>().deltaMove -= deltaMove;
				}
			}
